...
service Controller {
  ...
  rpc GetTopologyKeys (GetTopologyKeysRequest)
    returns (GetTopologyKeysResponse) {}
...
message PluginCapability {
  message Service {
    enum Type {
      ...
      // cluster. The CO MUST pass topology information to the driver
      // using CreateVolumeRequest and use the topology information
      // returned by NodeGetInfo and GetTopologyKeys to ensure that a
      // given volume is accessible from a given node when scheduling
      // workloads.
...
message CreateVolumeRequest {
  ...
  // If this field is specified and at least one of its topology keys
  // (see `message Topology`) is unrecognized by the SP (i.e. missing
  // from the list returned by `GetTopologyKeys()`), the CreateVolume()
  // call MUST return error code INVALID_ARGUMENT.
...
message Volume {
  ...
  // node.
  // If this field is specified, its topology keys (see `message
  // Topology`) in this field MUST be included in the list returned by
  // `GetTopologyKeys()`.
  //
  // Example 1:
...
// Topology is a map of topological domains (or topology keys) to
// topological segments (or topology values).
// A topological domain is a sub-division of a cluster, like "region",
// "zone", "rack", etc.
// A topological segment is a specific instance of a topological domain,
// like "zone3", "rack3", etc.
// For example {"com.company/zone": "Z1", "com.company/rack": "R3"}
// Valid topology keys have two segments: an optional prefix and name,
// separated by a slash (/), for example: "com.company.example/zone".
...
A Controller Plugin MUST implement this RPC call if it has `ACCESSIBILITY_CONSTRAINTS` plugin capability.
The RPC allows the CO to query the list of topology keys recognized by the SP.
The driver MUST recognize and generate instances of `Topology` containing only keys in this list, i.e. there should never be a key inside an instance of `Topology` used by the driver that is missing from this list.


```protobuf
message GetTopologyKeysRequest {
  // Intentionally empty.
}

message GetTopologyKeysResponse {
  // The list of topology keys recognized by the SP.
  // This field is REQUIRED.
  repeated string topology_keys = 1;
}
```

##### GetTopologyKeys Errors

If the plugin is unable to complete the GetTopologyKeys call successfully, it MUST return a non-ok gRPC code in the gRPC status.
